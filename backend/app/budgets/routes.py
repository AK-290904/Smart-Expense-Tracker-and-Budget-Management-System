...
def get_budget_alerts():
    ...
    try:
        ...
        for budget in budgets:
            ...
            budget_float = float(budget.amount)
            percentage = (spent_float / budget_float * 100) if budget_float > 0 else 0
            ...
            if percentage > 100:
                alert_level = 'danger'
                message = f"You've exceeded your {category.name} budget by ₹{spent_float - budget_float:.2f}!"
            elif percentage >= 90:
                alert_level = 'warning'
                message = f"You've used {percentage:.0f}% of your {category.name} budget"
            elif percentage >= 75:
                alert_level = 'info'
                message = f"You've used {percentage:.0f}% of your {category.name} budget"
            ...
            if alert_level:
                alerts.append({
                    ...
                    'budget_amount': budget_float,
                    'spent_amount': spent_float,
        ...
        return jsonify({'error': 'Failed to fetch budget alerts', 'message': str(e)}), 500
